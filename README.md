...
## 1.40.0 Part 1~7 비즈니스 영어 실전

- 영어 코스의 `학습 → 실전 → 비즈니스 영어 실전`에서 Part 1~7을
  모두 풀 수 있다.
- 빠른 10문제, 파트별 연습, 120분 200문제 실전, 틀린 문제 재도전을
  복잡한 설정 없이 바로 시작한다.
- 듣기 대본은 기기의 영어 음성으로 재생하고, 연습 모드는 답을 고른 즉시
  정답 근거와 선택지별 풀이를 보여 준다.
- 실전 모드는 풀이를 채점 후에만 열고, 타이머·답안지·다시 볼 문제·미응답
  경고를 제공한다. 진행 중 답안은 로컬 DB에 즉시 저장된다.
- `exam-packs/`에 독자 제작 200문제, JSON Schema, SHA-256 카탈로그와
  재생성·검증 도구를 둔다. 이후에는 앱 재배포 없이 문제팩을 추가할 수 있다.
- 문제는 Sprache용으로 새로 작성한 자료이며 공식·기출문제를 포함하지 않는다.
  상표 및 비제휴 고지는 문제팩 정보에 함께 표시한다.
- 자세한 내용은 [1.40.0 릴리스 노트](docs/release-notes-1.40.0.md)에 있다.

## 1.39.0 공부부터 시작하는 단순한 흐름

- 홈의 `무료 자료`와 자료실 `추가`의 첫 번째 항목에서 현재 언어의
  추천 학습 자료를 바로 받을 수 있다.
- 언어팩 화면은 현재 언어만 먼저 보이고, 다른 언어와 출처·파일 정보는
  필요할 때만 펼칠 수 있게 했다.
- 다운로드 뒤에는 복잡한 파일 가져오기 도구 대신 추가될 개수와
  `추가하기` 버튼을 먼저 보여 준다.
- 모바일 자료실의 그룹·선택 버튼을 하나로 합치고, 중복 정리·휴지통은
  자료 관리 메뉴로 옮겼다.
- Drive 미연결 안내, 0개인 자동 모음, 모바일 검색 예시를 상시 화면에서
  숨겨 학습 자료가 먼저 보이게 했다.
- `운영자 서버`, `SHA-256`, `오류 보고서` 같은 사용자 흐름과 관계없는
  표현을 제거하고 실제 동작에 맞는 짧은 안내로 바꾸었다.
- 자세한 내용은 [1.39.0 릴리스 노트](docs/release-notes-1.39.0.md)에 있다.

## 1.38.9 빠른 추가와 바로 보이는 언어팩

- 자료 합성 결과를 언어별로 캐시해 `추가` 메뉴와 자료 화면이 열릴 때마다
  같은 목록을 다시 만들지 않게 했다.
- 전체 검색은 입력을 잠시 기다린 뒤 자료를 나누어 검색하며, 새 검색이 들어오면
  이전 검색을 취소해 타이핑과 화면 전환이 멈추지 않게 했다.
- 빠른 추가 화면은 커서 이동만으로 전체 화면을 다시 그리지 않고, 중복·예문·태그
  추천도 입력이 멈춘 뒤 계산한다.
- `GitHub 언어팩 받기`를 자료실의 추가 메뉴에 큰 항목으로 배치하고, 전체 메뉴
  검색에서도 `언어팩` 또는 `language pack`으로 바로 열 수 있게 했다.
- 영어·일본어·독일어·프랑스어·스페인어·중국어 팩은 GitHub에서 내려받은 뒤
  파일 크기와 SHA-256을 확인하고 가져오기 검토 화면으로 연결한다.
- 자세한 내용은 [1.38.9 릴리스 노트](docs/release-notes-1.38.9.md)에 있다.

## 1.38.8 더 단순한 학습 화면

- 학습 첫 화면은 `뜻풀이`, `뜻 보고 단어 선택`, `문장 맞추기` 세 가지만
  바로 보여 주고 저장된 기본값으로 즉시 시작한다.
- 추천, 미션, 카드, 듣기, 발음, 시험과 세부 게임 설정은
  `더 많은 학습 방식` 안으로 옮겨 필요할 때만 펼친다.
- 뜻을 보고 외국어 단어를 고르는 역방향 선택형 문제를 정식 학습 방식으로
  추가하고, 앱 재실행과 플레이리스트에서도 방향이 유지되게 했다.
- 설정 첫 화면은 검색과 항목 목록만 남기고, Drive·화면·학습량·개인정보의
  긴 설명과 조작은 각 항목을 연 뒤에만 보이도록 정리했다.
- 중복된 바로 이동·전체 초기화 버튼과 최근 학습 카드의 중복 실행 버튼,
  학습 화면의 장황한 소개 문구를 제거했다.
- 설정의 앱 정보에서 Android APK와 iPhone PWA 설치 링크를 한 번에
  시스템 공유창으로 보낼 수 있고, 공유창을 쓸 수 없으면 링크를 복사한다.
- 자세한 내용은 [1.38.8 릴리스 노트](docs/release-notes-1.38.8.md)에 있다.

## 1.38.7 실제 언어팩과 조용한 Drive 동기화

- Git 공개 카탈로그에 TUFS CC BY 4.0 기반 언어팩 6개를 실제로 등록했다.
  앱 내장 어휘와 팩 내부 중복을 제외해 영어 352, 일본어 402, 독일어 361,
  프랑스어 360, 스페인어 354, 중국어 370개가 새로 추가된다.
- `자료실 → 추가 → 언어팩 받기`에서 현재 언어 팩을 바로 확인하고, 크기와
  SHA-256을 검증한 뒤 기존 가져오기 검토 화면에서 선택해 설치한다.
- 백그라운드·앱 복귀·자동 재시도 Drive 동기화는 화면 메시지를 띄우지 않는다.
  로그인·연결과 사용자가 직접 누른 동기화만 진행 상태와 결과를 보여 준다.
- 출처 정렬 자료와 내장 중복 제외 목록, 팩 재생성 도구를 함께 보관해 이후에는
  앱 재배포 없이 Git 언어팩만 추가하거나 교정할 수 있다.
- 자세한 내용은 [1.38.7 릴리스 노트](docs/release-notes-1.38.7.md)에 있다.
...
- 로그인 없이 여섯 언어의 내장 단어 480개·문장 600개(언어별 100개)를 바로 학습
